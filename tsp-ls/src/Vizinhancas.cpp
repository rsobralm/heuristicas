...
void swap(int ils, int maxIter, vector<int> &melhoras, double &totalmelhoras, vector<int> &solution, double &cost, int n, vector<int> &positionList, double **adjMatrix, vector<vector<int>> &arrangedMatrix)
{
  ...
  if(ils == 0){ // swap na primeira iteracao;
    ...
    {  
      for (int j = 1; j < n; j++)       // pega os 20 vizinhos mais proximos
      {
  ...
  else{ // swap no restante

    int posI = -1, posJ = -1;                             // guarda as posições para realizar a operação
    for (int i = 2; i < solution.size() - 2; i++)           // exclui da operação a ultima e penultima posição do vetor
    {  
      for (int j = 1; j < maxIter; j++)                           // pega os 20 vizinhos mais proximos
      {
